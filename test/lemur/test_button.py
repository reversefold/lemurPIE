...
class TestButton(mox.MoxTestBase):
	...
	def test_initial_state(self):
		self.assertEquals(self.button.tick(), Button.State.UP)
	...
	def test_transition_up_to_pressed(self):
		...
		self.assertEquals(self.button.tick(), Button.State.PRESSED)

	def test_transition_pressed_to_down(self):
		self.test_transition_up_to_pressed()
		self.assertEquals(self.button.tick(), Button.State.DOWN)
	...
	def test_transition_down_to_released(self):
		self.test_transition_pressed_to_down()
		self.controller.a = False
		self.assertEquals(self.button.tick(), Button.State.RELEASED)
	...
	def test_state_repeatable(self):
		self.test_transition_released_to_pressed_2()
		self.assertEquals(self.button.state, Button.State.PRESSED)
		self.assertEquals(self.button.state, Button.State.PRESSED)
		self.button.tick()
		self.assertEquals(self.button.state, Button.State.DOWN)
		self.controller.a = False
		self.assertEquals(self.button.state, Button.State.DOWN)
		self.assertEquals(self.button.tick(), self.button.state)
		self.assertEquals(self.button.state, Button.State.RELEASED)
